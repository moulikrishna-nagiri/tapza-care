...
export function setMockEmptyResponses(
  values: Partial<
    Pick<
      typeof mockSettings,
      "emptyDoctors" | "emptySlots" | "emptyPrescriptions"
    >
  >,
) {
...
export function loadMockSettings(): Promise<void> {
  if (!mockSettingsReady) {
    mockSettingsReady = AsyncStorage.getItem(MOCK_SETTINGS_KEY).then(
      (value) => {
        if (!value) return;
        try {
...
async function mockDelay(milliseconds: number) {
  ...
  await delay(
    mockSettings.networkMode === "slow" ? milliseconds * 3 : milliseconds,
  );
}
...
function isMockSettings(value: unknown): value is MockSettings {
  ...
  return (
    (candidate.networkMode === "normal" ||
      candidate.networkMode === "slow" ||
      candidate.networkMode === "failed") &&
...
export async function fetchAppConfig(
  variant: AppConfigVariant = "normal",
): Promise<LayoutConfig> {
  await mockDelay(650);
  if (shouldFailRequest())
    throw new Error("The configuration service is unavailable.");
...
export async function getDoctors(): Promise<Doctor[]> {
  await mockDelay(500);
  if (shouldFailRequest())
    throw new Error("The doctor service is unavailable.");
...
export async function getSlots(
  ...
  return times.map((time, index) => {
    ...
    return {
      ...
      startsAt,
      endsAt,
      available: !bookedSlotIds.has(id) && index !== 3,
    };
  });
}
...
export async function createBooking(
  ...
  await mockDelay(700);
  if (shouldFailBooking || shouldFailRequest())
    throw new Error("We could not complete your booking.");
  if (mockSettings.bookingConflict || bookedSlotIds.has(slotId)) {
    const error = new Error("This slot is no longer available.") as Error & {
      status: number;
    };
    error.status = 409;
    throw error;
  ...
  return {
    ...
    slotId,
    status: "confirmed",
    date: slotId.match(/\d{4}-\d{2}-\d{2}/)?.[0] ?? "",
  };
}
...
export async function getPrescriptions(): Promise<Prescription[]> {
  await mockDelay(550);
  if (shouldFailRequest())
    throw new Error("The prescription service is unavailable.");
