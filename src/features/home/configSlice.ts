  ...
  async (options) => {
    ...
    try {
      ...
      throw error instanceof Error
        ? error
        : new Error("Unable to load home configuration.");
    }
  },
  {
    condition: (_, { getState }) =>
      !(getState() as { config: ConfigState }).config.loading,
  },
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loadConfig.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.error.message ?? "Unable to load your home screen.";
      });
  },
});
