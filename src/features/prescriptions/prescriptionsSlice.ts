...
export const loadPrescriptions = createAsyncThunk<Prescription[]>('prescriptions/load', async () => normalizePrescriptions(await getPrescriptions()), {
...
export const loadDoseStatuses = createAsyncThunk<Record<string, boolean>>('prescriptions/loadDoseStatuses', async () => {
  ...
  try {
    ...
    return isRecord(parsed) ? Object.fromEntries(Object.entries(parsed).filter(([, status]) => typeof status === 'boolean')) as Record<string, boolean> : {};
  } catch {
    return {};
  }
});
...
function normalizePrescriptions(value: unknown): Prescription[] {
  ...
  return value.filter(isRecord).map((item, prescriptionIndex) => {
    ...
    return {
      id: typeof item.id === 'string' ? item.id : `prescription-${prescriptionIndex + 1}`,
      doctorName: typeof item.doctorName === 'string' ? item.doctorName : '',
      doctorSpecialty: typeof item.doctorSpecialty === 'string' ? item.doctorSpecialty : '',
      patientName: typeof item.patientName === 'string' ? item.patientName : '',
      date: typeof item.date === 'string' ? item.date : '',
      diagnosis: typeof item.diagnosis === 'string' ? item.diagnosis : '',
      medicines: rawMedicines.filter(isRecord).map((medicine, medicineIndex) => ({
        id: typeof medicine.id === 'string' ? medicine.id : `medicine-${prescriptionIndex + 1}-${medicineIndex + 1}`,
        name: typeof medicine.name === 'string' ? medicine.name : '',
        dosage: typeof medicine.dosage === 'string' ? medicine.dosage : '',
        duration: typeof medicine.duration === 'string' ? medicine.duration : '',
        instructions: typeof medicine.instructions === 'string' ? medicine.instructions : '',
        schedule: {
          morning: isRecord(medicine.schedule) && medicine.schedule.morning === true,
          afternoon: isRecord(medicine.schedule) && medicine.schedule.afternoon === true,
          night: isRecord(medicine.schedule) && medicine.schedule.night === true,
        },
      })),
    };
  });
...
const prescriptionsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loadPrescriptions.fulfilled, (state, action) => { state.loading = false; state.prescriptions = action.payload; state.error = null; })
