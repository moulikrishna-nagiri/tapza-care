...
export default function BookingsScreen() {
  ...
  return <PlaceholderScreen title="My Bookings" actionLabel="Book an Appointment" onAction={() => router.push('/booking' as RelativePathString)}>{!bookings.length && <Text style={[placeholderStyles.heading, { color: appTheme.colors.text }]}>No upcoming appointments</Text>}{bookings.map((booking) => <View key={booking.id} style={[styles.booking, { backgroundColor: appTheme.colors.surface, borderColor: appTheme.colors.border }]}><Text style={[styles.name, { color: appTheme.colors.text }]}>{booking.doctor?.name ?? 'Doctor appointment'}</Text><Text style={[styles.specialty, { color: appTheme.colors.mutedText }]}>{booking.doctor?.specialty}</Text><Text style={[styles.date, { color: appTheme.colors.text }]}>{new Date(`${booking.date}T12:00:00`).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}</Text><Text style={[styles.time, { color: appTheme.colors.text }]}>{booking.slot ? new Date(booking.slot.startsAt).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }) : ''}</Text><Text style={[styles.status, { color: appTheme.colors.success }]}>Confirmed</Text></View>)}</PlaceholderScreen>;
}
