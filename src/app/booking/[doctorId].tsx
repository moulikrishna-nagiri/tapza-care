...
export default function DoctorBookingScreen() {
  ...
  useEffect(() => {
    dispatch(setSelectedDoctor(doctorId ?? null));
    dispatch(setSelectedDate(date));
  }, [date, dispatch, doctorId]);
  useEffect(() => {
    if (doctorId && date) dispatch(loadSlots({ doctorId, date }));
  }, [date, dispatch, doctorId]);
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.center}>
          ...
          <Text style={styles.subtitle}>
            Please return and choose another doctor.
          </Text>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.subtitle}>
          {doctor.specialty} · INR {doctor.feeInr.toLocaleString("en-IN")}{" "}
          consultation
        </Text>
        <Text style={styles.sectionTitle}>Select date</Text>
        <DateSelector dates={dates} selectedDate={date} onSelect={handleDate} />
        <Text style={styles.sectionTitle}>Select time</Text>
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Open available appointment times"
        ...
        {!selectedSlot && error && (
          <View>
            ...
            <Pressable
              accessibilityRole="button"
              accessibilityLabel="Retry loading available slots"
              onPress={() => dispatch(loadSlots({ doctorId: doctor.id, date }))}
              style={styles.retry}
            >
        ...
        {!selectedSlot && !loadingSlots && !slots.length && (
          <Text style={styles.empty}>
            No appointments available for this date. Try another date.
          </Text>
        )}
        {loadingSlots && (
          <View style={styles.loading}>
      ...
      <SlotBottomSheet
        ...
        onSelect={handleSlot}
        onConfirm={() => setSheetVisible(false)}
...
const styles = StyleSheet.create({
  ...
  timeButtonText: { color: theme.colors.text, fontSize: 15, fontWeight: "700" },
  chevron: { color: theme.colors.primary, fontSize: 28 },
  selectedNote: {
    padding: 12,
    borderRadius: 12,
  ...
  loading: { gap: 12, marginTop: 16, padding: 20 },
